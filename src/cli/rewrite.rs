...
///
/// Reads all records from the input file, sorts them by ordinal for
/// monotonicity, then writes them to a new file with the requested page
/// configuration. Dead pages and alignment waste are eliminated. The
/// output file is written to a `.buffer` temporary file and renamed on
/// success to ensure atomicity.
pub fn run(
    input: &str,
    output: &str,
