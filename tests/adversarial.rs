...
/// Exporting with `--as-is` should not add missing newlines.
#[test]
fn test_export_as_is() {
    let tmp = NamedTempFile::new().unwrap();
...
/// Exporting without `--as-is` adds missing newlines (default behavior).
#[test]
fn test_export_adds_newlines_by_default() {
    let tmp = NamedTempFile::new().unwrap();
    let slab_path = tmp.path().to_path_buf();
